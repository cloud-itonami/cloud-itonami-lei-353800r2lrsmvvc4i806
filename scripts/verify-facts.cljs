...
;; pinned at 2d0ece0c21b16e9e8a736e2a0affdd784de25461. Do not edit here.
;;
...
;;
;; ## This file is the canonical copy. The archive repos vendor it.
;;
;; It lives here, beside `lei-acquire.cljs` and the rest of the family's
;; generators, because this superproject is what creates and registers the
;; `cloud-itonami/cloud-itonami-lei-*` repos. Each of those repos carries a
;; byte-identical copy at `scripts/verify-facts.cljs` under a VENDORED header
;; naming this path and a commit.
;;
;; The copies are not a mistake to be cleaned up. An archive repo's value is
;; that a single `git clone` of it can be verified against the live registry
;; with no workspace, no west, and no dependency resolution. Deleting the copy
;; and depending on this file would take that away. So the copy stays, and
;; `scripts/verify-vendored-copies.cljs` performs the diff instead.
;;
;; Everything the script needs about a particular company it reads from that
;; repo's `blueprint.edn` (`:company/lei`) -- there is nothing per-company in
;; here, which is why one canonical is possible at all.
;;
